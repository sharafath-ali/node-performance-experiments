...
How do we prove _where_ the CPU is stuck instead of just guessing? We use **Clinic.js**.
...
- _In the Flamegraph, look for massive, wide horizontal bars. Those indicate synchronous functions monopolizing the CPU._

### ✅ Fixing the Bottleneck

How do we fix it? We move the heavy synchronous work _off_ the main thread.
...
Generate requests, and eventually close the server. Clinic generates a visualization showing exactly _which structures_ allocated the memory. You will clearly see `Array` or `(string)` rapidly climbing the heap.
